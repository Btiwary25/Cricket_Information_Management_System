#include <stdio.h>
// RUN Function 2 Before Semi-Finals
// Check cases for n=2,4
//No Use For match_id in player struct
// Made match_id integer
char man_Of_All_Matches[25] = "";
char highest_Total_Individual_Run_Player[25] = "";
// Correct code for multiple centuries ,"k times"
//Function for sort_name_playeres max individual run before semifinals
// sort_team_points
//Runs in last 5 matches
// MOTM IN match()
//present MOTM()
// sort_MOTM(MOTM_Players, 0, MOTM_Player_count); //In decreasing order,on basis of runs ???????? part 7
...
int main()
{
    ...
    for (int i = 0; i < total_teams; i++)
    {
        ...
        for (int j = 0; j < 15; j++)
        {
            ...
            // printf("\nEnter match_id\t");
            // scanf(" %s", &team_list[i].lineup[j].match_id);
    ...
    // You can execute the function for highest run before knockout matches(Function 2)
